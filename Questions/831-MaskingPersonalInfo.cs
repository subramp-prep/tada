...
namespace questions
{
    ...
    //To mask an email, all names must be converted to lowercase and all letters between the first and 
    ...
    //form "+***-***-***-1111".  The '+' sign and the first '-' sign before the local number 
    //should only exist if there is a country code.For example, a 12 digit phone number mask should start with "+**-".
    ...
    public class MaskingPersonalInfo
    {
        public static string MaskPII(string S)
        {
            ...
            {
                // Email ID mask
                output = char.ToLower(S[0]).ToString();
